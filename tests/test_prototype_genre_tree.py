...
def test_load_seed_tree_creates_criteria_from_fixture(api_client, user, criteria_type):
...
def test_load_seed_tree_replaces_existing_criteria(api_client, user, criteria_type):
...
def test_load_seed_tree_missing_file_raises(api_client, criteria_type, settings, tmp_path):
    settings.DATA_DIR = tmp_path

    with pytest.raises(FileNotFoundError):
        api_client.post("/genre-criteria/tree/load-seed/")


def test_load_seed_tree_calls_on_loaded_hook(api_client, user, criteria_type, monkeypatch):
    calls = []

    monkeypatch.setattr(GenreCriteriaViewSet, "on_seed_tree_loaded", lambda self, request: calls.append(request.user))
